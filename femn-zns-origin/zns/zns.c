...
#define BLK_MAXIMUM_ERASE_COUNT 30;
...
static void zns_aio_zone_reset_cb(NvmeRequest *req, NvmeZone *zone)
{
    ...
    uint64_t num_lun = zns->num_lun;
    struct ppa ppa;
    uint32_t reset_zone_id = zns_zone_idx(ns, zone->d.zslba);
    uint32_t reset_block_id;
    uint32_t chip_offset;
    for (ch = 0; ch < num_ch; ch++) {
        for (lun = 0; lun < num_lun; lun++) {
            reset_block_id = (ch*128)*8 + (lun*32)*8 + reset_zone_id;
            ppa.g.ch = ch;
            ppa.g.fc = lun;
            ppa.g.blk = zns_zone_idx(ns, zone->d.zslba);
            chip_offset = lun  + ch * num_lun;
            femu_debug("chip_id:%u-reset_block_id:%u\n\r",chip_offset,reset_block_id);
