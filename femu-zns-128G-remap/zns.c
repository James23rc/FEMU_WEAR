...
// //4g默认一个lun是32个blks 32G:256个Blk 128G:1024个BLK 
#define BLKS_PER_LUN 1024
#define BLKS_PER_CH 4096
#define BLKS_PER_ZONE 32
// #define ERASE_THRESHOLD 32
#define REMAP_BETWEEN_ZONES 32

static inline void zns_zone_blks_remaping(FemuCtrl *n, NvmeZone *zone,uint32_t zone_id);
static inline void zns_init_zone_blks_map(FemuCtrl *n);
...
static inline struct ppa lpn_to_ppa(FemuCtrl *n, NvmeNamespace *ns, uint64_t lpn)
{
    ...
    // zone_blk_offset = wpp->ch + wpp->lun*zns->num_ch;
    // ppa.g.blk = zone.blks[zone_blk_offset]->blk_id;
    //这里应该是去写zone里面的blk
    // zone_blk_offset = wpp->ch + wpp->lun*zns->num_ch;
    // if (zone.blks[0] != NULL)//说明有映射
    // {
    //     ppa.g.blk = zone.blks[zone_blk_offset]->blk_id;
    //     // femu_debug("zone_blk_offset:%u,blk_id:%lu\n\r",zone_blk_offset,zone.blks[zone_blk_offset]->blk_id);
    //     zone.blks[zone_blk_offset]->is_empty = 0;
    //     advance_read_pointer(n);
    // }else{//无映射不应该读和写 以及 移动指针
    //     femu_debug("zone:%u remaping blk donot exist\n\r",zone_idx);
    // }
...
static void zns_aio_zone_reset_cb(NvmeRequest *req, NvmeZone *zone)
{
    ...
    for (ch = 0; ch < num_ch; ch++) {
        for (lun = 0; lun < num_lun; lun++) {
            ...
            zone_blk_offset = ch  + lun * num_ch;//0 8 16 24 1 9 17 15
            // if (zone->blks[zone_blk_offset]->is_empty == 0)
            // {
                zone->blks[zone_blk_offset]->erase_count += 1;
                zone->erase_count += 1;
                // zns->ch[ch].fc[lun].erase_count += 1;暂时没用到
                chip_offset = (zone->blks[zone_blk_offset]->blk_id/BLKS_PER_LUN);
                femu_debug("chip_id:%u-reset_block_id:%u,erase_count:%u\n\r",chip_offset,zone->blks[zone_blk_offset]->blk_id,zone->blks[zone_blk_offset]->erase_count);
                struct nand_cmd erase;
                erase.cmd = NAND_ERASE; 
                erase.stime = 0;
                zns_advance_status(n, &erase, &ppa);
            // }else{
            //     zone->blks[zone_blk_offset]->is_empty = 1;
            // }
        }
    }
    femu_debug("reset_zone_id:%u,reset_count:%lu\n\r",reset_zone_block_id,zone->erase_count);
}
...
static void zns_init_blk(struct zns_blk *blk, uint8_t ch_id, uint8_t lun_id, uint16_t blk_offset)
{
    blk->erase_count = 0;
    /*
...
static void zns_init_zone_blks_map(FemuCtrl *n)
{
    ...
    for (int i = 0; i < n->num_zones; i++)
    {
        ...
        for (int j = 0; j < BLKS_PER_ZONE; j++)
        {
            ...
            // ssd->ch[wpp->ch].fc[wpp->lun].blk[zone_idx].blk_id;
            // femu_debug()
            //初始化blk存储池
    ...
    // for (int i = 250; i < n->num_zones; i++)
    // {
    //     uint32_t zone_idx = zns_zone_idx(ns, zones[i].d.zslba);
    //     // femu_debug("zone_id:%u映射初始化成功\n\r",zone_idx);
    //     for (int m = 0; m < BLKS_PER_ZONE; m++)
    //     {
    //         femu_debug("zone_id:%u映射Blk:%u\n\r",zone_idx,zones[i].blks[m]->blk_id);
    //     }
    // }
    // ssd->map_exist = 1;
...
static void zns_zone_blks_remaping(FemuCtrl *n, NvmeZone *zone ,uint32_t zone_id)
{
    ...
    if(zone->blks[0] == NULL || (zone->erase_count - ssd->zns_blk_pool.zone_min_erase_count) >= REMAP_BETWEEN_ZONES)//|| zone->erase_count >= ERASE_THRESHOLD
    {
        ...
        for (int ch= 0; ch < ssd->num_ch; ch++)
        {
            for (int lun = 0; lun < ssd->num_lun; lun++)
            {
                ...
                for (int n = 0; n < BLKS_PER_LUN; n++)
                {
                    ...
                    //  if(ssd->ch[ch].fc[lun].blk[n].is_empty == 1)
                    {
                ...
                // femu_debug("select ch:%u lun:%u blk_id:%u ec:%u -> zone:%u\n\r",ch,lun,min_erase_blk->blk_id,min_erase_blk->erase_count,zone_id);
...
static void zns_init_params(FemuCtrl *n)
{
    ...
    id_zns->map_exist = 0;
    // femu_debug("BLKS_PER_CH:%u",BLKS_PER_CH);
    memset(id_zns->zns_blk_pool.blks_pool, 0, sizeof(uint8_t) * BLKS_PER_CH * id_zns->num_ch);
    femu_debug("id_zns->num_ch:%lu,lun_num:%lu\n\r",id_zns->num_ch,id_zns->num_lun);
