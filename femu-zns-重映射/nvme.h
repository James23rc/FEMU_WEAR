...
typedef struct NvmeRequest {
    ...
    NvmeCmd                 cmd;            //该次请求所包含的命令信息
    NvmeCqe                 cqe;		    //该次请求的返回队列
    uint8_t                 cmd_opcode;
    QEMUSGList              qsg;
    QEMUIOVector            iov;
    QTAILQ_ENTRY(NvmeRequest)entry;
    int64_t                 stime;          //记录接受到该请求的时间（实时时间）
    int64_t                 reqlat;         //该次请求的延迟
    int64_t                 gcrt;
    int64_t                 expire_time;	//该次请求的响应结束时间，等于stime + reqlat
...
typedef struct FemuCtrl {
    ...
    char            devname[64];
    struct rte_ring **to_ftl; //无锁环，ftl从这里面去数据
    struct rte_ring **to_poller;//无锁环，poller从这里取数据
    pqueue_t        **pq;          //优先级队列，poller都有一个to_ftl、to_poller和pq
    bool            *should_isr;
    bool            poller_on;
    ...
    uint8_t         multipoller_enabled;        //多轮询器使能
    uint32_t        nr_pollers;             //轮询器的数量:目前是1
